...
class PaymentServiceClientHandler  {
    ...
    async setDefaultCard(token, req) {
        try {
            ...
            const response = await fetch(process.env.PAYMENT_SERVICE_SET_CARD_DEFAULT_URL+`/${cardId}/default`, {
                method: 'POST',
                headers: { 'Content-Type' : 'application/json', 'Authorization': `Bearer ${token}`, },                
    ...
    async deleteCard(token, req) {
        try {
            let cardId = req.params.cardId;
            const response = await fetch(process.env.PAYMENT_SERVICE_DELETE_CARD_URL+`/${cardId}`, {
                method: 'DELETE',
                headers: { 'Content-Type' : 'application/json', 'Authorization': `Bearer ${token}`, },  
                body: JSON.stringify(req.body)              
            });
