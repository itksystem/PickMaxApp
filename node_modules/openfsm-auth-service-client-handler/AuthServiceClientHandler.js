...
/*
  ...
  */
  ...
  async setSecurityQuestion(req) {
    try {        
        const response = await fetch(process.env.AUTH_SET_TWO_FACTOR_URL, {
            method: 'POST',
            headers: this.headers(req),            
            body: JSON.stringify(req.body)
        });
        const data = await response.json();
        if (response.ok) {
            console.log(`setSecurityQuestion success.`);
            return { success: true, data };
        } else {
            console.log(`setSecurityQuestion failed.`);
            return { success: false, status: response.status, data };
        }
    } catch (error) {
        console.log(`setSecurityQuestion failed.`);
  ...
  async pinCodeLogon(req) {
    try {        
        const response = await fetch(process.env.AUTH_PIN_CODE_LOGON_URL, {
            method: 'POST',
            headers: this.headers(req),            
            body: JSON.stringify(req.body)
        });
        const data = await response.json();
        if (response.ok) {
            console.log(`pinCodeLogon success.`);
            return { success: true, data };
        } else {
            console.log(`pinCodeLogon failed.`);
            return { success: false, status: response.status, data };
        }
    } catch (error) {
        console.log(`pinCodeLogon failed.`);
