...
class WarehouseServiceClientHandler {
    ...
    async removeItemFromBasket(req) {
        try {
...
async createOrder(req) { // Сформировать заказ из товаров в корзине
 ...
 async getOrderDetails(req, orderId) {
  ...
  async removeItemFromOrder(req) {
    try {
          const response = await fetch(process.env.WAREHOUSE_SERVICE_REMOVE_ITEM_FROM_ORDER_URL, {
            method: 'POST',                
            headers: this.headers(req),
            body: JSON.stringify(req.body)
        });
...
/*
     * Получение категорий  MC WAREHOUSE
     * @params {token} - токен пользователя
     * @params {orderId} - ID запроса
     * @returns {Object} - Объект с параметрами заказа
     */

async  getProductCategories(req) {   
  try {
    const response = await fetch(process.env.WAREHOUSE_SERVICE_GET_PRODUCT_CATEGORIES_URL, 
       {
        method: 'GET',
        headers: this.headers(req),              
    });          
...
async  getProductBrands(req) {   
    try {
      const response = await fetch(process.env.WAREHOUSE_SERVICE_GET_PRODUCT_BRANDS_URL, 
         {
          method: 'GET',
          headers: this.headers(req),              
      });          
  ...
  async  getSearchQueries(req) {
    try {
          const response = await fetch(process.env.WAREHOUSE_SERVICE_SEARCH_QUERIES_URL, {
            method: 'POST',                
            headers: this.headers(req),
            body: JSON.stringify(req.body)
        });
