class ConfirmationPhoneSection extends PageBuilder {
    ...
    /**
     * Generates the ConfirmationPhone section module.
     ...
     */
    ConfirmationPhoneCardContainer() {
        this.ConfirmationPhoneContainer = document.createElement("div");
        this.ConfirmationPhoneContainer.className = "card confirmation-code-container";

        const ConfirmationPhoneContainerHeader = document.createElement("div");
        ConfirmationPhoneContainerHeader.className = "card-header";
        ConfirmationPhoneContainerHeader.innerHTML = `<h3 class="card-title">Подтверждение телефона</h3>`;

        const ConfirmationPhoneContainerContent = document.createElement("div");
        ConfirmationPhoneContainerContent.className = "card-body";
        ConfirmationPhoneContainerContent.innerHTML = `
        ...
        this.ConfirmationPhoneContainer.appendChild(ConfirmationPhoneContainerHeader);
        this.ConfirmationPhoneContainer.appendChild(ConfirmationPhoneContainerContent);

        this.addModule("ConfirmationPhone", this.ConfirmationPhoneContainer);
     ...
     */
    async getConfirmationPhoneCode(confirmationType = 'phone') {
        const webRequest = new WebRequest();
     ...
     * @returns {Promise<Object>} - Response from the server
     */
    async sendConfirmationPhoneCode(code, requestId) {
        const webRequest = new WebRequest();
        return await webRequest.post(
     ...
     */
    addEventListeners() {
        const getConfirmationCodeButton = this.ConfirmationPhoneContainer.querySelector('.get-confirmation-code-button');
        const sendConfirmationCodeButton = this.ConfirmationPhoneContainer.querySelector('.send-confirmation-code-button');
        const backProfileButton = this.ConfirmationPhoneContainer.querySelector('.back-confirmation-code-button');

        const infoLabel = this.ConfirmationPhoneContainer.querySelector('.confirmation-information-label-container');
        const confirmationCodeElement = this.ConfirmationPhoneContainer.querySelector('confirmation-code');
        ...
        getConfirmationCodeButton?.addEventListener('click', async () => {
            try {
                ...
                const request = await this.getConfirmationPhoneCode('phone');
        ...
        sendConfirmationCodeButton?.addEventListener('click', async () => {
            try {
                ...
                const request = await this.sendConfirmationPhoneCode(code, this.requestId);
