class geoDrawEditor{
  ...
  getCoorditatesString(arr){ // выдать массив геоточек в формате хранилища 
     let pathstr = "";
     let _s="";
     for (var i = 0; i < arr.length; i++) {
            let lat = parseFloat(arr[i][0]);
            let lng = parseFloat(arr[i][1]);
            pathstr += _s+lat+' '+lng;
	   _s=",";
     }
    pathstr += "";
  ...
  stopEditing(){
    this.Polygon.editor.stopEditing(); // включить редактор 	  
  }

  startEditing(){
    this.Polygon.editor.startEditing(); // включить редактор 	  
  }
