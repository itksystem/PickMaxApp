class ProductCard extends HTMLElement {
    constructor() {
        ...
        linkElem.setAttribute('href', '/src/components/ui/product-card/css/product-card.css');
        this.shadowRoot.appendChild(linkElem);
        
        const linkElem2 = document.createElement('link');
        linkElem2.setAttribute('rel', 'stylesheet');
        linkElem2.setAttribute('href', '/src/pages/plugins/adminlte/css/adminlte.min.css');
        this.shadowRoot.appendChild(linkElem2);
        
        const linkElem3 = document.createElement('link');
        linkElem3.setAttribute('rel', 'stylesheet');
        linkElem3.setAttribute('href', '/src/components/ui/basket-button/css/basket-button.css');
    ...
    static get observedAttributes() {
        return [
            'like', 'basket-count', 'product-id', 'href', 'status',
            'image-src', 'image-alt', 'video-src', 'brand', 'name',
            'current-price', 'currency-type', 'old-price', 'discount',
            'wallet-condition', 'aria-label'
        ];
    }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        switch(name) {
            case 'like':
                let likeWrap = this.shadowRoot.querySelector('.like-link');
                if (newValue == 1) likeWrap.classList.add('liked');
                let likePath = likeWrap.querySelector('path.like');
            ...
            case 'basket-count':
                ...
                if (_basketButton) _basketButton.setAttribute('basket-count', newValue || '');
                break;
                
            case 'product-id':
                this.productId = newValue;
                const basketButton = this.shadowRoot.querySelector('.button-add-to-basket');
                if (basketButton) basketButton.setAttribute('product-id', newValue);
                break;
            ...
            case 'video-src':
                ...
                if (productImage) productImage.setAttribute(name, newValue || '');
                break;
    ...
    connectedCallback() {
        ProductCard.observedAttributes.forEach(attr => {
