	...
	return this.each( function() {
		...
		var ts= this, grid={
			...
			dragStart: function(i,x,y) {
				...
				console.log({display:"block",left:x.clientX-gridLeftPos,top:y[1],height:y[2]});
...
$.jgrid.extend({
	...
	showHideCol : function(colname,show) {
		return this.each(function() {
			...
			if(fndh===true) {
				console.log($t.p.tblwidth); 	
				console.log($t.p.width); 	 
				console.log($t.p.shrinkToFit); 	 
			...
			$.each($t.p.colModel,function(ci){
				...
				if(this.search){
					...
					if(soptions.clearSearch) {
						...
						$("td:eq(2)",stbl).append("<a title='"+csv+"' style='padding-right: 0.3em;padding-left: 0.3em;' class='clearsearchclass' colindex='"+ci+"'>"+p.resetIcon+"</a>");
					} else {
						$("td:eq(2)", stbl).hide();
					}
					switch (this.stype)
