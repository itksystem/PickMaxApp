        ...
        _createArrowContours: function () {
            var contours = [],
                ...
                arrowLength = calculateArrowLength(
                    mainLineCoordinates,
                    this.options.get('arrowMinLength', 17),
            ...
            var arrowStep = this.options.get('arrowMaxStep', 10);
            if (arrowLength > 0) {
                for (var j=mainLineCoordinates.length-1; j>0; j-=1){
                // Создадим 2 контура для стрелочек.
                    var lastTwoCoordinates = [
                            mainLineCoordinates[j-1],
                            mainLineCoordinates[j]
                        ],
                        segmentLength = calculateSegmentLength(lastTwoCoordinates),
                    ...
                    // а потом развернем результаты обратно.
                        ...
                        arrowAngle = this.options.get('arrowAngle', 15) / 180 * Math.PI,
                    ...
                    //добавим длину нового отрезка к аккумулятору
                    lengthAccumulator+=segmentLength;
                    if(lengthAccumulator>arrowStep){
                        //если аккумулятор переполнен, отнимем степ
                        //и отрисуем стрелку
                        lengthAccumulator -=arrowStep;
                    }
                    else if(j<mainLineCoordinates.length-1)
                    {
                        //Иначе, если это не первая (носовая) стрелка,
                        //мы её поскипаем.
                        continue;
                    }
