...
class ObjectPropertiesComponent {
  ...
  constructor(userId, config) {
      console.log('ObjectPropertiesComponent load...');
      let o = this;
      userId=sessionStorage.getItem('userId');
      o.user=new User(userId);
      o.userId = userId;
      o.config = config;
      o.mapPoligon = new Map('objectMap'); 
      o.mapPoligon.init();
      o.loadObjectProp(config.objectId);
 }
...
// выводим пагинацию
  changePagination(data){
   ...
   if(data.pages != null)
    ...
    pagination+='<li class="page-item"><a class="page-link profile-changes-object-nav last" page="'+data.pagesCount+'" href="#">>></a></li></ul>';
    return pagination;
   }
...
/* устанавливаем обработчик на кнопку просмотра описания изменений */
 ...
 saveObjectHandler(){
  $(document).off("click","#change-object-name-btn-click").on("click","#change-object-name-btn-click",function(e) {
    e.preventDefault();  
  ...
  loadObjectProp(objectId){
    console.log("loadObjectProp");
    let o=this;
/* Отключаем кнопки */
    $("#change-object-name-btn-click").hide();  // Прячем кнопки "Изменить" "Cancel" которые не используются при управлении обьектом
    $("#change-object-close-btn-click").hide(); // в модальном окне из списка запросов
...
/* удаляем временные периоды на форме */
    ...
    $("#object-editor-dialog").show("fast");
     $.getJSON('/object/'+objectId, function(data) {
     try {
        console.log(data);
	...
	console.log(guid);
        ...
        $('#object-id').val((objectId != 'null') ? data.object.id : guid);
        $('#object-search-code').val((objectId != 'null') ? data.object.search_code : guid);
...
//
	  ...
	  .setMarker([parseFloat($('#object-latitude').val()).toFixed(6),parseFloat($('#object-longitude').val()).toFixed(6)],
   	    	{ iconContent:  data.object.search_code, objectType : "OBJECT"
	   });
...
//           ya.setCenter(data.center.latitude, data.center.longitude,5 );
...
// вывод геоточек
	      $.ajax({
	  	  url: "/servicecalls/"+o.config.callId,
 		  cache: false,
		  method: "GET",
		  dataType: "json",
		  success: function(_o){ 
	       	    console.log(_o);
		    let user_id=_o.entity.user.id;
	   	    let finishdate=_o.entity.dates.finishdate;
	            o.track=new geoTrack({date: finishdate, user_id: user_id, storageTrack : "object-properties-component-track"}).onMap(o.mapPoligon.Map);

	  	   //  Отразить транспортный трек

                   let routeTrip =  new RouteTrip();
                       routeTrip.setCallId(o.config.callId) // пробрасываем в обьект routeTrip модели пользователя и обслуживаемого обьекта	
		      .setFinishDate(_o.entity.dates.finishdate)
                      .setObject(data.object)
		      .onMap(o.mapPoligon.Map, user_id, finishdate); 

	               ya.setCenter([parseFloat($('#object-latitude').val()).toFixed(6),parseFloat($('#object-longitude').val()).toFixed(6)]);

	  	       let _local_place_ = new UserLocalPlace(o.mapPoligon.Map, _o.entity.user.id);
  		 	   _local_place_.onMap();
  	        	   routeTrip.setUser(_local_place_.user.entity.user);
...
// Обработка RangeSlider - увеличение кругового радиуса при GEOZONE_CUSTOM_RADIUS_TYPE
 ...
 try{
  ...
  me.initialization(()=>{
	 console.log('ObjectPropertiesComponent loading....');
